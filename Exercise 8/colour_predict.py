...
def rgb_to_lab(X):
    return color.rgb2lab(X.reshape(1, 1, -1)).reshape(-1)
...
def main():
    ...
    bayes_convert_model = make_pipeline(FunctionTransformer(converter), GaussianNB())
    knn_rgb_model = KNeighborsClassifier(n_neighbors=12)
    knn_convert_model = make_pipeline(FunctionTransformer(converter), KNeighborsClassifier(n_neighbors=9))
    rf_rgb_model = RandomForestClassifier(min_samples_split=10)
    rf_convert_model = make_pipeline(FunctionTransformer(converter), RandomForestClassifier(min_samples_split=10))
